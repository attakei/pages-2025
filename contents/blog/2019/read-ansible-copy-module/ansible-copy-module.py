...
def clear_facls(path):
    ...
    rc, out, err = module.run_command(
        b_acl_command, environ_update=dict(LANG="C", LC_ALL="C", LC_MESSAGES="C")
    )
    if rc != 0:
        raise RuntimeError(
...
def copy_common_dirs(src, dest, module):
    ...
    for item in common_dirs:
        ...
        changed = changed or copy_common_dirs(
            os.path.join(src, item), os.path.join(dest, item), module
        )
    return changed
    ...
    if checksum_src != checksum_dest or os.path.islink(b_dest):
        if not module.check_mode:
            try:
                ...
                module.atomic_move(
                    b_mysrc, dest, unsafe_writes=module.params["unsafe_writes"]
                )
